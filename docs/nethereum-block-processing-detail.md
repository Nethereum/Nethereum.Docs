...
Criteria can be synchronous or async.  Criteria dictates whether or not your action is invoked. Async criteria allows you to do dynamic lookups which may involve external calls to registries/databases/web services etc.  For instance, whilst processing you may need to check dynamic registries as part of your criteria and naturally these calls tend to be async.  Synchronous criteria allows you to inject in-memory criteria easily.

#### Speed Tip (Place your Criteria carefully!)
Each step occurs in order and is dependant on the previous step.  The earlier you can place criteria, the faster the processor will be because it will prevent retrieving or crawling irrelevant data.  
...
Create a repository factory - this will create the necessary Azure Storage Tables and provide a repositories that write to them.  You'll need an Azure connection string.  The second argument "samples" is a table prefix.  You can use the prefix to separate Blockchain data from different sources or for different purposes using the same Azure storage account.
``` csharp
var repoFactory = new AzureTablesRepositoryFactory(_azureConnectionString, "samples");
```

Create the processor passing in the repository factory.  By default block progress will be stored in a azure table.
``` csharp
